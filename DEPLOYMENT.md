...
- **운영 서버**: Oracle Cloud, `ssh investory`, 경로: `/opt/investory`
- systemd: `investory`
- 리버스 프록시: Caddy
- 도메인: `https://investory.kro.kr`

## ⚠️ 빌드는 반드시 AWS에서!
investory 서버는 RAM 503MB라서 Next.js 빌드 시 OOM 발생함.
**절대로 investory에서 `next build` 하지 말 것.**
...
# 3. 재시작
ssh investory "sudo systemctl restart investory"
```

## 자동 배포 (OpenClaw cron)
- 매일 아침 8시 KST, TSLA/BMNR 분석 리포트 생성 → AWS 빌드 → investory 배포
